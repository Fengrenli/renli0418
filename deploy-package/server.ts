...
// GET /api/list-materials (BOM 材料库)
app.get('/api/list-materials', async (req, res) => {
  try {
 ...
 */
app.post('/api/bom/calculate-scene', async (req, res) => {
  try {
    const { scene, waste_rate, assembly_code } = req.body;
    ...
    const matResult = await pool.query(
      'SELECT code, name, price, unit, category FROM materials WHERE code = ANY($1)',
      [allSkus]
    );
