...
function inferGeometry(sku: string, name?: string): THREE.BufferGeometry {
  ...
  if (lower.includes('瓦') || lower.includes('tile') || lower.includes('abeth') || lower.includes('x8d5')) {
    // 瓦片 → 扁平弧面（用薄长方体近似）
    return new THREE.BoxGeometry(0.3, 0.02, 0.2);
  }
  if (lower.includes('脊') || lower.includes('ridge') || lower.includes('abhrg')) {
    // 脊 → 半圆柱（用圆柱近似）
    return new THREE.CylinderGeometry(0.08, 0.08, 1.2, 8);
  }
  if (lower.includes('砖') || lower.includes('brick') || lower.includes('abmrr')) {
    // 砖 → 标准长方体
    return new THREE.BoxGeometry(0.24, 0.06, 0.12);
  }
  if (lower.includes('柱') || lower.includes('column') || lower.includes('pillar')) {
    // 柱 → 圆柱
    return new THREE.CylinderGeometry(0.15, 0.18, 3.0, 12);
  }
  if (lower.includes('梁') || lower.includes('beam')) {
    // 梁 → 长方体
    return new THREE.BoxGeometry(0.2, 0.3, 2.5);
  }
  if (lower.includes('滴水') || lower.includes('abtie')) {
    // 滴水 → 小三角（用锥体近似）
    return new THREE.ConeGeometry(0.06, 0.1, 4);
  }
  if (lower.includes('猫头') || lower.includes('abmao')) {
    // 猫头 → 小球
    return new THREE.SphereGeometry(0.08, 8, 8);
  }
...
const ArchitectureNode: React.FC<ArchitectureNodeProps> = ({ node }) => {
  ...
  // 占位几何体（确保有 groups 防止 raycast materialIndex 崩溃）
  const geometry = useMemo(() => {
    const geo = inferGeometry(node.sku, node.name);
  ...
  useFrame((_, delta) => {
    ...
    if (isSelected || isPulsing) {
      ...
      (mesh.material as THREE.MeshStandardMaterial).emissive = new THREE.Color('#ff4444');
      (mesh.material as THREE.MeshStandardMaterial).emissiveIntensity = 0.3 + Math.sin(Date.now() * 0.008) * 0.2;
    } else if (hovered) {
