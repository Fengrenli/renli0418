...
const ProjectTimeline: React.FC<ProjectTimelineProps> = ({ lang, project, onStatusChange, onTimelineChange }) => {
  ...
  // 仅在切换项目时初始化，避免父组件回写触发反复重置（导致“疯狂跳”）
  React.useEffect(() => {
    ...
    const initialStageStatuses: Record<string, 'pending' | 'inProgress' | 'completed'> = {
      design: 'inProgress',
      procurement: 'inProgress',
      logistics: 'inProgress',
      installation: 'inProgress',
      operation: 'inProgress',
    };
    const initialSubStepStatuses: Record<string, 'pending' | 'inProgress' | 'completed'> = {};

    stageOrder.forEach((stageId, stageIndex) => {
      const stageSubSteps = t.subSteps[stageId];
      stageSubSteps.forEach((step, subIndex) => {
        initialSubStepStatuses[`${stageId}-${step.id}`] = stageIndex === 0 && subIndex === 0 ? 'inProgress' : 'pending';
      });
    });

    if (project && Array.isArray(project.stages)) {
      project.stages.forEach((rawStage: any, idx: number) => {
        const mappedStageId = stageOrder[idx] || rawStage?.id;
        if (!mappedStageId) return;
        initialStageStatuses[mappedStageId] = normalizeStatus(rawStage?.status) === 'completed' ? 'completed' : 'inProgress';

        const expectedSteps = t.subSteps[mappedStageId as keyof typeof t.subSteps] || [];
        if (Array.isArray(rawStage?.subSteps)) {
          rawStage.subSteps.forEach((rawSubStep: any, subIdx: number) => {
            const mappedStepId = expectedSteps[subIdx]?.id || rawSubStep?.id;
            if (!mappedStepId) return;
            initialSubStepStatuses[`${mappedStageId}-${mappedStepId}`] = normalizeStatus(rawSubStep?.status);
          });
        }
      });
    }
    ...
    setSubStepStatuses(initialSubStepStatuses);
  ...
  React.useEffect(() => {
    const builtStages = stageOrder.map(stageId => ({
      ...
      status: stageStatuses[stageId] || 'inProgress',
    ...
    const projectStatus: '进行中' | '已完成' = completedStages === stageOrder.length ? '已完成' : '进行中';
